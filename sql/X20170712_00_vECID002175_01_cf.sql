USE [CreditFulfillment] --<<-- database name
GO
...
GO
--SELECT * FROM version WHERE version = 2175.01
--DELETE version WHERE version = 2175.01
...
DECLARE	 @Name  VARCHAR(50)				= 'ECID'		--<<-- regular build, databasename  IF an OCR (not part of build) Request nnnnnn WHERE nnnnn = ticket number
		...
		, @Comment VARCHAR(512)			= 'Create Name to Gender table and populate it'			--<<-- please put in a short comment that makes it easy to find IF the occasion arises
...
BEGIN 
	...
	BEGIN TRY
		...
		CREATE TABLE Decision.NameToGender(
			Name varchar(25) NOT NULL,
			Gender CHAR(1) NOT NULL,
			CreateDate DATETIME DEFAULT(GETDATE())
		CONSTRAINT [PK_NameToGender_Name] PRIMARY KEY CLUSTERED(Name ASC) 
		...
		FROM 'C:\Imports\X20170712_00_vECID002175_01_cf.txt'
		WITH   (BATCHSIZE      = 5000 
				,DATAFILETYPE   = 'char'
				,FIRSTROW       = 1
				,FIELDTERMINATOR = '|'
		...
		PRINT 'BULK INSERT #TempTable ' + lower(@@rowcount) + ' rows expected 90699 rows.';
			
		INSERT Decision.NameToGender (Name, Gender)
		SELECT Name, Gender
		FROM #TempTable p;
			
		PRINT '<<<INSERT Decision.NameToGender ' + lower(@@rowcount) + ' rows>>>';
